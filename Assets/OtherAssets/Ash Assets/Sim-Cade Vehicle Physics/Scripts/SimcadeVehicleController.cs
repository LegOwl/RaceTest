...
namespace Ashsvp
{
    public class SimcadeVehicleController : MonoBehaviour
    {
        ...
        private void Update()
        {
            var inputDirection = controls.Player.Move.ReadValue<Vector2>();
            var inputBreak = controls.Player.Jump.ReadValue<float>();
            if (CanDrive && CanAccelerate)
            {
                
                accelerationInput = inputDirection.y;
                steerInput = inputDirection.x;
                brakeInput = inputBreak;
            }
